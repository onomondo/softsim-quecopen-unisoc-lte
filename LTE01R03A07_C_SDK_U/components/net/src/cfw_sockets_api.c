...
static void _dataIndNotify(void *ctx)
{
    ...
    {
        OSI_LOGI(0, "_dataIndNotify socket:%d retrn", socket);
        return;
    }
    if (func == NULL)
    {
...
bool sys_post_event_to_APS(void *msg, uint32_t msg_type)
{
    if (netThreadID != NULL)
    {
        ...
        return osiEventTrySend(netThreadID, &ev, 0);
    }
    return false;
}
...
extern bool wan_setPriority(uint8_t nSimId, uint8_t nCid, uint8_t nPriority);
extern uint8_t wan_getPriority(uint8_t nSimId, uint8_t nCid);
bool CFW_Set_Netif_Wan_Priority(uint8_t nSimId, uint8_t nCid, uint8_t nPriority)
{
    return wan_setPriority(nSimId, nCid, nPriority);
}
uint8_t CFW_Get_Netif_Wan_Priority(uint8_t nSimId, uint8_t nCid)
{
    return wan_getPriority(nSimId, nCid);
}