...
LOG_RAMCODE void boot_osiTraceExSpec(unsigned tag, unsigned partype, unsigned nargs, const char *fmt, ...)
{
    /*
    #if !QUEC_ONLY_ENABLE_LOGI
    if (!gTraceEnabled)
    ...
    va_list ap;
    va_start(ap, fmt);
    prvTraceExSpec(tag, partype, fmt, ap, nargs);
    va_end(ap);
    #endif
 */
    return;
...
//add 2024.4.17 裁剪log,
LOG_RAMCODE void boot_osiTraceIdExSpec(unsigned tag, unsigned partype, unsigned nargs, unsigned fmtid, ...)
{
/*
    #if !QUEC_ONLY_ENABLE_LOGI
    if (!gTraceEnabled)
    ...
    va_list ap;
    va_start(ap, fmtid);
    prvTraceIdExSpec(tag, partype, fmtid, ap, nargs);
    va_end(ap);
    #endif
 */
...
//add 2024.4.17 裁剪log,
OSI_DECL_STRONG_ALIAS(boot_osiTraceExSpec,void osiTraceExSpec(unsigned tag, unsigned partype, unsigned nargs, const char *fmt, ...));
OSI_DECL_STRONG_ALIAS(boot_osiTraceIdExSpec,void osiTraceIdExSpec(unsigned tag, unsigned partype, unsigned nargs, unsigned fmtid, ...));
