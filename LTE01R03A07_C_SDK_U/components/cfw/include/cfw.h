...
#define CFW_BLACK_WHITE_PLMN_NUM 10
typedef struct
{
...
uint32_t CFW_EmodRfDigGpioPwrCfg(int16_t txpwr_dbm, uint16_t rfctrl_idx);
...
uint32_t CFW_GetLTERealTimeInfo(uint8_t *pNetinfo, uint16_t *nLength, uint8_t nStep, CFW_SIM_ID nSimID);
...
typedef enum
{
    APN_DEFAULT = 0, //activate default hosting or commom data service.
    APN_MMS = 1,     //send and receive multimedia message
    APN_IMS = 5,     //register volte used
    APN_XCAP = 8,    //additional business
    APN_SOS = 9,     //emergency volte call
} CFW_APN_ACCOUNT_TYPE;
//get apn from ats/apn: global/local apn .csv
uint32_t CFW_GetApnInfo_V2(CFW_APNS_UNAME_UPWD *apn, uint8_t plmn[7], CFW_APN_ACCOUNT_TYPE apnType);
...
uint32_t CFW_NwSetEnableImsip(
    uint8_t bEnable,
    CFW_SIM_ID nSimID);
uint32_t CFW_NwGetEnableImsip(
    uint8_t *pEnable,
    CFW_SIM_ID nSimID);

uint32_t CFW_NwSetUseImsip(
    uint8_t bUse,
    CFW_SIM_ID nSimID);
...
#ifdef CONFIG_QUEC_PROJECT_FEATURE  //ql add, Unisoc patch
#define CMN_MM_PD     5    //MM     /* TS24.007 11.2.3.1.1 Protocol discriminator */
#define CMN_GMM_PD    8    //GMM
#define CMN_SM_PD     10   //SM
#endif

typedef struct
{
    uint8_t nPD; // MM_MMC_PD  CMN_MMP_PD
    uint8_t nMsg;
    uint8_t nCause;
    uint8_t nBsic;
    uint16_t nArfcn;
...
uint32_t CFW_SmsSetSpecialDcsFlag(bool status);
bool CFW_SmsGetSpecialDcsFlag(void);

uint32_t CFW_SimReadLargePreferPLMN(uint16_t nUTI, CFW_SIM_ID nSimID);
uint32_t CFW_FreeDynamicMemory(void *mem_address);

#define CFW_EPLMN_NUM 10
typedef struct
{
    uint8_t num;
    uint8_t ehplmn[6 * CFW_EPLMN_NUM];
} CFW_EHPLMN_T;
uint32_t CFW_NwGetEHPlmn(uint8_t *plmn, CFW_EHPLMN_T *pEHPlmn);
