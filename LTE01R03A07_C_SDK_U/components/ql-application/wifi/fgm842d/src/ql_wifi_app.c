...
typedef struct
{
    ...
    ql_ble_gatt_data_cb gatt_data_cb;
...
{
    ql_wifi_app_manager_s *manager = &ql_wifi_app_manager;
    unsigned char data = QL_WIFI_APP_RESET_CMD;

    ql_rtos_mutex_try_lock(ql_wifi_app_mutex);
    // Send wifi control command
    if (manager->output_data_msg_cb)
    {
        ql_wifi_msg_output_data_info_s info = {.cmd = QL_WIFI_APP_RESET_CMD, .data = &data, .data_len = 1};
        manager->output_data_msg_cb(&info);
    }
    ql_rtos_mutex_unlock(ql_wifi_app_mutex);
    return QL_WIFI_APP_SUCCESS;
...
ql_wifi_app_errcode_e ql_wifi_app_psm(void *argv)
{
    ...
    quec_wifi_psm_info *psm_info = (quec_wifi_psm_info *)argv;
    if(psm_info->dtim < 1 || psm_info->dtim > 30){
        return QL_WIFI_APP_INVALID_PARAM_ERR;
    }
    ...
    if (manager->output_data_msg_cb)
    {
        if(psm_info->enable == QUEC_WIFI_PSM_ENABLE)
        {
            //要进入低功耗模式，必须在DHCP完成后通知wifi模组已经获取到ip地址
            ql_wifi_msg_output_data_info_s info = {.cmd = QL_WIFI_APP_STA_SET_GOT_IP, .data = NULL, .data_len = 0};
            manager->output_data_msg_cb(&info);
        }

        QL_WIFI_APP_LOG("dtim = %d", psm_info->dtim);
        ql_wifi_msg_output_data_info_s info = {.cmd = QL_WIFI_APP_PSM_CMD, .data = (uint8_t *)psm_info, .data_len = sizeof(quec_wifi_psm_info)};
        manager->output_data_msg_cb(&info);
    }
...
ql_wifi_app_errcode_e ql_ble_init(void *argv)
{
    ...
    ql_ble_app_service_info *service_info = (ql_ble_app_service_info *)argv;
    if(service_info->uuid_len != 2 && service_info->uuid_len != 16){
        return QL_WIFI_APP_INVALID_PARAM_ERR;
    }
    ...
    if (manager->output_data_msg_cb)
    {
        ql_ble_app_init_info app_init_data = {
            .role = 0 //目前只支持模块为外围设备,即参数0
        };
        ql_wifi_msg_output_data_info_s info = {
            .cmd = QL_BLE_APP_INIT_CMD, 
            .data = (uint8_t *)&app_init_data, 
            .data_len = sizeof(ql_ble_app_init_info)
        };
        manager->output_data_msg_cb(&info);

        info.cmd = QL_BLE_APP_SET_SERICE_UUID_CMD;
        info.data = (uint8_t *)service_info;
        info.data_len = sizeof(ql_ble_app_service_info);
        manager->output_data_msg_cb(&info);
    }
    ql_rtos_mutex_unlock(ql_wifi_app_mutex);
    return QL_WIFI_APP_SUCCESS;
...
ql_wifi_app_errcode_e ql_ble_set_characteristic(void *argv)
{
    ...
    {
        ql_wifi_msg_output_data_info_s info = {
            .cmd = QL_BLE_APP_SET_CHARACT_UUID_CMD, 
            .data = (uint8_t *)service_info, 
            .data_len = sizeof(ql_ble_app_service_info)
        };
        manager->output_data_msg_cb(&info);
    }
    ql_rtos_mutex_unlock(ql_wifi_app_mutex);
...
{
    ql_wifi_app_manager.gatt_data_cb = cb;
    return QL_WIFI_APP_SUCCESS;
}
...
ql_wifi_app_errcode_e ql_ble_set_adv_data(void *argv)
{
    ...
    ql_ble_adv_data_info *adv_data = (ql_ble_adv_data_info *)argv;
    if(adv_data->adv_data_len == 0 ||  adv_data->adv_data_len > 28){
        return QL_WIFI_APP_INVALID_PARAM_ERR;
    }
    ...
    {
        ql_wifi_msg_output_data_info_s info = {
            .cmd = QL_BLE_APP_ADV_DATA_CMD, 
            .data = (uint8_t *)adv_data, 
            .data_len = adv_data->adv_data_len + 1 //加1是因为adv_data->adv_data_len本身占用1字节的长度.
            };
        manager->output_data_msg_cb(&info);
    }
    ql_rtos_mutex_unlock(ql_wifi_app_mutex);
...
ql_wifi_app_errcode_e ql_ble_disconnect(void *argv)
{
    ql_wifi_app_manager_s *manager = &ql_wifi_app_manager;
    ql_rtos_mutex_try_lock(ql_wifi_app_mutex);
    // Send wifi control command
...
ql_wifi_app_errcode_e ql_ble_sent_gatt_data(void *argv)
{
    ...
    ql_ble_send_data_info *gatt_data = (ql_ble_send_data_info *)argv;
    if(gatt_data->uuid_len != 2 && gatt_data->uuid_len != 16){
        return QL_WIFI_APP_INVALID_PARAM_ERR;
    }
    ...
    {
        ql_wifi_msg_output_data_info_s info = {
            .cmd = QL_BLE_APP_DATA_SEND_CMD, 
            .data = (uint8_t *)gatt_data, 
            .data_len = gatt_data->data_len + 20 //20=len(uuid_len) + len(uuid) + len (data_len)=2+16+2
        };
...
static void ql_wifi_app_thread(void *argv)
{
    ...
    while (1)
    {
        ...
        case QL_BLE_APP_CONNECTED_RPT: {
            QL_WIFI_APP_LOG("ble connected");
            break;
        }

        case QL_BLE_APP_DISCONNECTED_RPT: {
            QL_WIFI_APP_LOG("ble disconnected");
            break;
        }

        case QL_BLE_APP_DATA_RECV_RPT: {
            QL_WIFI_APP_LOG("ble recv data:");
            ql_rtos_mutex_try_lock(ql_wifi_app_mutex);
            if (manager->gatt_data_cb != NULL)
            {
                manager->gatt_data_cb(cmd_info.msg_info);
            }
            ql_rtos_mutex_unlock(ql_wifi_app_mutex);
            break;
        }

        case QL_BLE_APP_MTU_CHANGE_RPT: {
            QL_WIFI_APP_LOG("ble mtu channged");
            break;
        }
        default: {
