...
// uart3
#define QL_WIFI_UART3_TX_PIN  QUEC_PIN_UART3_TXD
#define QL_WIFI_UART3_TX_FUNC 0x04
#define QL_WIFI_UART3_RX_PIN  QUEC_PIN_UART3_RXD
#define QL_WIFI_UART3_RX_FUNC 0x04

#define QL_WIFI_UART_PORT    QL_UART_PORT_3
#define QL_WIFI_UART_TX_PIN  QL_WIFI_UART3_TX_PIN
#define QL_WIFI_UART_TX_FUNC QL_WIFI_UART3_TX_FUNC
#define QL_WIFI_UART_RX_PIN  QL_WIFI_UART3_RX_PIN
#define QL_WIFI_UART_RX_FUNC QL_WIFI_UART3_RX_FUNC
...
void ql_wifi_uart_cb(unsigned int ind_type, ql_uart_port_number_e port, unsigned int size)
{
    ...
    QL_WIFI_UART_LOG("UART port %d ind type:0x%x, data size:%d", port, ind_type, size);
    switch (ind_type)
    {
    ...
    case QUEC_UART_RX_RECV_DATA_IND: {
        while (size > 0)
        {
            ...
            OSI_LOGXI(OSI_LOGPAR_M, 0, "Uart Rx : %*s", read_len, recv_buff);
...
int ql_wifi_uart_write(char *data, int data_len)
{
    ...
    // OSI_LOGXI(OSI_LOGPAR_M, 0, "Uart Tx : %*s", data_len, data);

    ql_uart_write(QL_WIFI_UART_PORT, (unsigned char *)data, data_len);
...
ql_wifi_uart_errcode_e ql_wifi_uart_init(void *argv)
{
    ...
    ql_wifi_uart_manager_s *manager = &ql_wifi_uart_manager;
    uart_cfg.baudrate = QL_UART_BAUD_2000000;
...
int ql_wifi_uart_output_data_msg(void *ctx)
{
    ...
    OSI_LOGXI(OSI_LOGPAR_M, 0, "uart tx: %*s", QUEC_PROTOCOL_HEAD_MAX_SIZE + data_len, output_data);
