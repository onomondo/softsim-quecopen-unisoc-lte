...
void osiTraceExSpec(unsigned tag, unsigned partype, unsigned nargs, const char *fmt, ...);
...
void osiTraceIdExSpec(unsigned tag, unsigned partype, unsigned nargs, unsigned fmtid, ...);
...
#define __OSI_LOGX(level, partype, fmtid, fmt, ...)                                                                             \
    ...
    {                                                                                                                           \
        ...
        {                                                                                                                       \
            if ((fmtid) == 0 || __OSI_LOG_DISABLE_ID)                                                                           \
                __OSI_LOGX_IMP((level << 28) | (OSI_LOCAL_LOG_TAG), partype, OSI_VA_NARGS(__VA_ARGS__), fmt, ##__VA_ARGS__);    \
            else                                                                                                                \
                __OSI_DLOGX_IMP((level << 28) | (OSI_LOCAL_LOG_TAG), partype, OSI_VA_NARGS(__VA_ARGS__), fmtid, ##__VA_ARGS__); \
        }                                                                                                                       \
    } while (0)
...
#define __OSI_LOGX_IMP(tag, partype, nargs, fmt, ...) osiTraceExSpec(tag, partype, nargs, fmt, ##__VA_ARGS__)
#define __OSI_DLOGB_IMP(tag, nargs, fmtid, ...) osiTraceIdBasic(tag, nargs, fmtid, ##__VA_ARGS__)
#define __OSI_DLOGX_IMP(tag, partype, nargs, fmtid, ...) osiTraceIdExSpec(tag, partype, nargs, fmtid, ##__VA_ARGS__)
